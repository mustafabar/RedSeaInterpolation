...
This script requires Python >= 2.6 and < 3.0.
...
def processTemplate(configfile) :
    ...
    if not config.has_section('Library') :
        print >> sys.stderr, 'Library section messing in specifications file', configfile;
        return False;
    ...
    for s in config.sections() :
        ...
        if len(argssplit) > 2 :
            print >> sys.stderr, 'At most one | allowed in arguments specification for function', s, ':', f['arguments'];
        ...
        # check that zeroripple is only used for univariate functions
        assert f['zeroripple'] == '0' or (f['argmin'] == 1 and f['argmax'] == 1);
    ...
    if 'C' in lib['languages'] :
        ...
        print >> outfile, template.render(lib = lib, funcs = funcs);
