...
Vmax_z     "maximal speed in the z direction [m/s]"           /0.05/
VmaxC                                                         /3/
dt                                                             /610/
...
if(Par(se,'ft') < BIGTF0,
   ...
   until card(hd) = 0   or timeelapsed > 1100;
...
file wdata2 /current_ex4a0.5.csv/;
*wdata2.ap = 1; {Flag to turn the append option on for the file with pointer wdata.}
...
if(Par(se,'ft') < BIGTF,
   ...
   until card(hd) = 0   or timeelapsed > 2100;
