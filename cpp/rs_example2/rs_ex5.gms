...
amax       "maximal acceleration [m/s^2]"                     /2/   
Vmax       "maximal speed [m/s]"                              /2/ 
amax_z      "maximal acceleration in the z direction [m/s^2]"  /0.5/
Vmax_z     "maximal speed in the z direction [m/s]"           /0.5/
VmaxC                                                         /3/
dt                                                             /610/
pos_x0     "intial position in x direction"                   /1700000/ 
pos_y0     "intial position in y direction"                   /258958/
pos_z0     "intial position in z direction"                   /-1800/
pos_xf     "final position in x direction"                   /1688658/ 
pos_yf     "final position in y direction"                   /162436/
pos_zf     "final position in z direction"                   /-1900/
V_x0       "intial velocity in x direction"                   /0/
V_y0       "intial velocity in y direction"                   /0/
V_z0       "intial velocity in z direction"                   /0/
SA         "current_Level"                                    /1/
cl                                                            /3/
ct                                                           /0.0/
stop                                                         /0.0/
...
loop(se,
     loop( t $(ORD(T) EQ ORD(se)+ min_tf),
		...
		pos.lo('x',t)=pos_xf-dt/2;
		pos.lo('y',t)=pos_yf-dt/2;
		pos.lo('z',t)=pos_zf;
		pos.up('x',t)=pos_xf+dt/2;		
		pos.up('y',t)=pos_yf+dt/2;
		pos.up('z',t)=pos_zf+dt/5;
...
if(Par(se,'ft') < BIGTF0,
   ...
   until card(hd) = 0   or timeelapsed > 1000;
...
loop(se,
     loop( t $ (ORD(T) EQ ORD(se)+ min_tf2),
            h(t)=1;
