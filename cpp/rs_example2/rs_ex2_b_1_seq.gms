...
file wdata /ini_ex2_b_1_seq.csv/;
...
while (ct gt 120,
* and stop = 0,
	loop(tt $(ORD(tt) EQ ct),
	 h(tt)=1;
	pos.lo('x',tt)=pos_xf-dt;
	pos.lo('y',tt)=pos_yf-dt;
	pos.lo('z',tt)=-3;
	pos.up('x',tt)=pos_xf+dt;
	pos.up('y',tt)=pos_yf+dt;
	pos.up('z',tt)=-2;


	solve path_planning0 minmizing loss using NLP;

   	Par(tt,'solv')= path_planning0.solvestat; 

   	Par(tt,'mode')= path_planning0.modelstat;

   	Par(tt,'tim')= path_planning0.resusd;
...
file wdata2 /current_ex2_b_1_seq.csv/;
*wdata2.ap = 1; {Flag to turn the append option on for the file with pointer wdata.}
...
execute_loadpoint 'path_planning0_p';
...
* and stop = 0,
	...
	solve path_planning2 minmizing loss using NLP;

   	Par(tt,'solv')= path_planning2.solvestat; 

   	Par(tt,'mode')= path_planning2.modelstat;

   	Par(tt,'tim')= path_planning2.resusd;
